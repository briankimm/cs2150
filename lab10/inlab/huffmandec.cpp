//Brian Kim, byk6q, 11/23/21, huffmandec.cpp
...
int main (int argc, char** argv) {
    ...
    while (true) {
        ...
        file >> prefix;
	//adds to the prefix bits and the char to a map
	decodeMap.insert(make_pair(prefix, character));
        // do something with the prefix code
	// cout << "character '" << character << "' has prefix code '" << prefix << "'" << endl;
    }
    /*
    unordered_map<string, string>:: iterator a;
    for(a = decodeMap.begin();a!=decodeMap.end();a++){
    ...
    for(int i = 0;i<allbits.length();i++){
      	empty+=allbits[i];
      if(decodeMap.find(empty) !=decodeMap.end()){
	cout<<decodeMap[empty];
	empty = "";
      }
    }
    cout<<endl;
