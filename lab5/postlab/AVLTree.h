//Brian Kim, byk6q, 10/9/21, AVLTree.h
...
private:
  ...
  void insert(AVLNode*& avl, const string& x);//insert helper method 
  string pathTo(AVLNode* avl, const string& x)const;//prints out the path To the node passed through
  bool find(AVLNode* avl, const string& x)const;//checks to see if the node is already in the tree
  int numNodes(AVLNode* avl) const;//keeps tracks of the number of Nodes
  void calcHeight(AVLNode*& avl);//void method to help calculate the height 
