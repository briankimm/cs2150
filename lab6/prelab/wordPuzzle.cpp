//Brian Kim, byk6q, 10/19/21, wordPuzzle.cpp
#include <iostream>
#include <string>
#include <fstream>
#include <vector>
...
bool checkprime(unsigned int p) {//method from primemethod.cpp
...
int getNextPrime (unsigned int n) {//method from primemethod.cpp
    while ( !checkprime(++n) );
    return n; // all your primes are belong to us
}
int main(int argc, char *argv[]){//main method
  hashTable hT = hashTable();
  ifstream file(argv[1]);//reads through file
  string s;
  timer t;
  if(file.is_open()){//checks to see if it was able to read through
  ...
  for(int r = 0; r< rows; r++){
    for(int c = 0; c<cols;c++){
      for(int d = 0; d < 8;d++){
	for(int l = 3;l <= 22; l++){
	  s = getWordInGrid(r, c, d, l, rows, cols);
	  //checks to see if the s is in the hashTable and if the length is the same
	  if(hT.contains(s) && s.length() == l){
	    if(d == 0){
	      direction = "N ";
	      wordsFound++;
	    }
	    ...
	    cout<< direction + "(" + to_string(r) + ", " + to_string(c) + "): " + s << endl;
	  }
